...
public class ItemList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/* 페이징기능 - 한 페이지에 보여줄 상품수 */
		int pagingSize = 10;
		
		/* 지금 보여주는 페이지의 순번 처리 request객체로 호출 및 저장 */
		int pagingNumber = 1;
		if(request.getParameter("pagingNumber")!=null) {
			pagingNumber = Integer.parseInt(request.getParameter("pagingNumber"));
		}
		
		int count = 0; // 카운터 변수(전체 상품수) 초기화
		int number = 0; // 넘버링 변수 초기화, 한 페이지에 출력하는 상품수
		
		ItemDAO dao = new ItemDAO(); // 상품 클래스 생성
		...
		request.setAttribute("count", count); // 전체 상품수
