...
public class CartUserDAO {
	...
	/* 카트 총수 구하기 */
	public int getAllNumbers(String id,String field,String search) {
		d.getCon();
		
		int count = 0;
		
		try {
			String sql = "select count(*) from cart where id='"+id+"'";
			/* 검색어가 있을 경우 검색된 총수를 구하는 쿼리문 */
			if(search!=null && !search.equals("")) {
				sql = "select count(*) from cart where id='"+id+"' and "+field+" like '%"+search+"%'";
			}
	...
	/* 카트목록 구하기 */
	public ArrayList<Cart> getAllMembers(String id,int startRow,int endRow,String field,String search){
		d.getCon();
