...
<table class="bodyFrame">
	...
	<tr>
		...
		<table width=100%>
			...
			<tr>
				...
				<th align=center style="width:200px;">첨부파일</th>
			</tr>
			<tr><td colspan=4 style="border-top:1px solid black"></td></tr>
			<%
			...
			<tr>
				...
				<td style="padding:5px;"><img src="/upload/<%=attach%>" width="100px;" height="70px;"><a href="view.jsp?code=<%=code%>&no=<%=no%>&field=<%=field%>&search=<%=search%>&page_now=<%=page_now%>"><%=title %></a><%if(!attach.equals("")){ %><img src="img/icon_file.gif"><%} %><%if(issueDate2.getTime() >= day2.getTime()){%><img src="img/main_new.gif"><%}%></td>
	...
	<tr>
		...
		<table width="100%">
			<tr>
				<td><a href="/bbs1/list.jsp?code=<%=code%>">[새로고침]</a></td>
				<!-- 관리자만 글을 쓸 수 있게 한다. -->
				<td align=right>
				<%if(session_id != null && session_level.equals("3")){ %>
					<a href="/bbs1/write.jsp?code=<%=code%>">[글쓰기]</a>
				<%}else { %>
