...
<table class="bodyFrame">
	...
	<tr>
		...
		<table class="bodyFrame">
			...
			int num = total_record - ((page_now-1) * num_per_page);
			int i=0; // 사진이 4장 이상일 때마다 개행을 하기 위한 변수
			
			while(rs_sort.next()){
				String no = rs_sort.getString("no"); // 게시물 번호
				String id = rs_sort.getString("id"); // 게시물 아이디
				String name = rs_sort.getString("name"); // 게시물 작성자 
				String title = rs_sort.getString("title"); // 게시물 내용
				String issueDate = rs_sort.getString("issueDate"); // 게시물 작성일
				String attach = rs_sort.getString("attach"); // 첨부파일
				...
				%>
					<td align=center style="padding:10px;"><img src="/upload/<%=attach%>" width="300px;" height=""><br>
					<a href="view.jsp?code=<%=code%>&no=<%=no%>&field=<%=field%>&search=<%=search%>&page_now=<%=page_now%>"><%=title %></a>
					<%if(!attach.equals("")){ %><img src="img/icon_file.gif"><%} %><%if(issueDate2.getTime() >= day2.getTime()){%>
	...
	<tr>
		...
		<table width="100%">
			<tr>
				<td><a href="/bbs3/list.jsp?code=<%=code%>">[새로고침]</a></td>
				<td align=right>
				<%if(session_id != null){ %>
					<a href="/bbs3/write.jsp?code=<%=code%>">[글쓰기]</a>
				<%}else { %>
